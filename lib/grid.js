...
class Grid {
    ...
    generateMazes() {
        ...
        var grid = this;        
        var starts = this.findMazeStarts(initialCells);
        
        while(starts.length > 0) {
            var newStart = starts.pop();
            grid.getCell(newStart.x, newStart.y).setValue(1);
            starts = grid.findMazeStarts(starts);
        }
    }
    

    findMazeStart(mazeCells) {
        return mazeCells.find(function(cell) {
            return cell.checkNeighbors(['north', 'south', 'east', 'west']) === 0;
        });
    }

    findMazeStarts(mazeCells) {
        return mazeCells.filter(function(cell) {
            return cell.checkNeighbors(['north', 'south', 'east', 'west']) === 0;
        });
    }
}
