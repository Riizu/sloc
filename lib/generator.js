class Generator {
    ...
    generateCorridors() {
        var usableRooms = this.grid.rooms;
        ...
        usableRooms.forEach(function(room) {
            var startPoint = mainRoom.cells[Math.floor(Math.random() * mainRoom.cells.length - 1) + 1];
            var endPoint = room.cells[Math.floor(Math.random() * room.cells.length - 1) + 1];
